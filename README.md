...
Na presente aula foram desenvolvidos 4 (quatro) estratégias básicas de trade automático e no sentido de estabelecer uma comparação, entre os setups, medidas clássicas de perfomance foram aplicadas: i) Quantidade de Operações, ii) Relação Retorno/Lucro, iii) Fator de Lucro, iv) Lucro e v) Taxa de Acerto. É notório que muitas outras medidas relevantes podem ser aplicadas (Exemplo: drawdown, fator de recuperação, payoff e etc). Naturalmente, novas medidas de desempenho serão inseridos no decorrer das atividades futuras
